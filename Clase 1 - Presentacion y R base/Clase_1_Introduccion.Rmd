...
Este curso busca dar las herramientas básicas para el procesamiento de estadísticas socio-laborales con R y la visualización de resultados con PowerBI y Looker. Cronogama: 
...
Como verán, el cronograma del el curso es *muy ambicioso* y cubre una amplia variedad de herramientas. El obetivo es conocer las potencialidades de todas estas herramientas para que ustedes luego profundicen según su interés o necesidades, siempre utilizando el procesamiento de estadísticas socio-laborales como referencia. 

Antes de empezar queremos llamar la atención de que la actividad de programar cambió drásticamente en los últimos años debido al desarrollo de modelos de lenguaje basados en IA. Actualmente, el foco del aprendizaje ya no debe estar puesto en conocer en profundidad los comandos, funciones y operadores de R sino más bien en saber qué es posible hacer con R y cómo aplicar las soluciones a los problemas que se nos presentan. 
...
- automatización y web scraping con R
- integración de R con GooglSheets

El curso está pensado para personas que no tienen ninguna experiencia en programación. Si ese es tu caso, te recomendamos que sigas detenidamente las primeras clases e intentes reproducir los comandos de esta página en tu computadora. Además te invitamos a que saques provecho de las herramientas basadas en IA para aprender a programar (sobre el final de este encuentro te compartimos algunos consejos). Ahora a empezar!
...
Hay muchas [opciones](https://docs.github.com/es/get-started/onboarding/getting-started-with-your-github-account#3-elegir-c%C3%B3mo-interactuar-con-github) para interactuar con GitHub. A continuación vamos a comentar dos de ellas que son las que recomendamos para usar en el curso. Sugerimos elegir al menos una de ellas para sumarse al repositorio del curso, aunque pueden utilizarse en simultáneo:


###   [Rstudio + Git](https://happygitwithr.com/rstudio-git-github.html)

Es la integración de las herramientas de Git y GitHub al IDE de RStudio. En el [tutorial que les pasamos]('Clase_1_Tutorial_instalaciones.html') se detalla cómo instalar Git, abrir una cuenta de GitHub y registrarse desde RStudio. En el 6to encuntro de este curso vamos a ver otras funcionalidades muy interesantes de GitHub. Hasta ahora lo único que necesitás saber es que haciendo click en el botón de Pull (flecha hacia abajo) vas a poder 'traer' las nuevas versiones del repositorio a la carpeta de tu compu.
...
- *ChatGPT*: conocidísimo modelo desarrollado por OpenAI. Tener en cuenta que, cuando traabajamos con R, debemos darle contexto al chat para que pueda darnos mejores respuestas. El contexto puede ser nuestro script entero o los mensajes que nos salen por consola (como por ejemplo el mensaje que nos sale cuando tenemos un error). También es una buena práctica dar una introducción en forma de texto sobre la actividad que estamos haciendo al menos una vez en un chat.
