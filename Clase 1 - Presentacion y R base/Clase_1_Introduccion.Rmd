...
- *1*: Introducción a R y Visual Studio Code
...
- Gratuito   
- Funciona principalmente por líneas de código
...
# Descripción de R y Visual Studio Code

Los lenguajes de programación se peuden ejecutar desde distintos entornos o IDEs. Un IDE es un programa que nos permite escribir código, ejecutarlo, y visualizar los resultados de manera más cómoda. En este curso vamos a trabajar principalmente con *Visual Studio Code*, aunque existe también otro IDE muy utilizado para trabajar con R, que es *RStudio*.

![https://code.visualstudio.com/](img/VSClogo.png)

*Visual Studio Code* es un programa desarrollado por Microsoft que es gratuito y de código abierto. Lo elegimos como el principal del curso por sus ventajas para la programación asistida con IA y su integración con Github.

![Pantalla Visual Studio Code](img/vsc_pantalla.png)

La barra de actividades de la izquierda contiene accesos rápidos a distintas funcionalidades del programa, entre ellas el control de versiones con Git, la terminal integrada, y la extensión de IA. El editor de código sirve para modificar los distintos archivos o scripts con los que trabajemos. Por último la terminal es donde se corren los comandos de R y se visualizan los resultados de los mismos.

![https://www.rstudio.com/](img/RStudiologo.png)

El el _programa_ __R studio__ fue por mucho tiempo fue el _entorno_ más cómodo para utilizar el _lenguaje_ __R__. Su interfaz es parecida a la de VSC aunque tiene algunas diferencias. Es un programa gratuito, aunque no de código abierto, desarrollado por la empresa Posit.
...
Visual Studio Code organiza el trabajo en **carpetas** que cumplen la misma función que los **proyectos** en RStudio.

La organización del trabajo en carpetas permite mantener un ambiente de trabajo delimitado por cada uno de nuestros proyectos. Es decir:
...
[GitHub](https://github.com/) es una plataforma diseñada para **almacenar y administrar código**, que al mismo tiempo nos permite llevar un *registro* y *control* de cualquier **cambio** sobre este código. Es una de las herramientas fundamentales para la **colaboración** y el **trabajo en equipo** en programación.
...
![build5nines.com](img/branching.png){width="60%"}


## Cómo interactuar con Git y GitHub?

Git funciona por medio de comandos que pueden insertarse por consola. Sin embargo, hay muchas opcines para interactuar con GitHub que facilitan el trabajo con Git sin necesidad de usar los comandos específicos de este programa.

###   [VSC + Git](https://happygitwithr.com/rstudio-git-github.html)

Git y GitHub se encuentran itegradas a los distintos IDE que existen para programar y Visual Studio Code no es la excepción. En el [tutorial que les pasamos]('Clase%201%20-%20Presentacion%20y%20R%20base/Clase_1_Tutorial_instalaciones.html') se detalla cómo instalar Git, abrir una cuenta de GitHub y registrarse desde Visual Studio Code. En el 6to encuentro de este curso vamos a ver otras funcionalidades muy interesantes de GitHub. 

Los repositorios de Github pueden 'clonarse' en nuestra computadora, lo que nos permite trabajar con los archivos del repositorio de manera local, y luego sincronizar los cambios que hagamos con el repositorio en la nube. Al acceder a VSC por primera vez o cuando abrimos una ventana nueva del programa, nos aparece esta página de bienvenida que nos permite rápidamente clonar un repositorio. Hacemos click en "Clone Git Repository" y pegamos la URL del repositorio que queremos clonar. En este caso, el repositorio del curso es: https://github.com/ceped-fce-uba/curso_aset.git

![Clonando repo](img/clonando_repo.png){width="80%"}

Haciendo click en el botón ```Sync changes``` vamos a poder recibir los cambios que hayan hecho otros integrantes del equipo, y enviar los cambios que hayamos hecho nosotros.

![Sincronizando cambios](img/git_en_vsc.png){width="40%"}

> Practica:     
1) Buscar un/a compañerx de curso y compartirle un repositorio creado por vos      
2) Pedirle a tu compañerx que clone el repositorio que le compartiste y que haga una modificación en el código (puede ser algo tan simple como agregar un comentario) y que luego sincronice los cambios.   
3) Sincronizar los cambios en tu computadora y verificar que la modificación que hizo tu compañerx se haya reflejado en tu repositorio local.

# Programar con IA

Hay numerosas herramientas para trabajar con modelos de lenguaje, y cada vez van apareciendo más. Visual Studio Code ofrece una integración muy cómoda con distintos modelos de lenguaje, a la cual se accede a través de la barra de actividades de la izquierda.

![IA](img/vsc_ia.png){width="40%"}

El asistente de IA de VSC se llama **Github Copilot** y es un producto desarrollado por Microsoft, que se encuentra disponible para distintos IDEs. Tiene la ventaja que la IA obtiene como contexto de trabajo los archivos con los que estamos trabajando, lo que hace que las sugerencias sean mucho más precisas.

Existen distintas modalidades de asistencia con IA ('Plan', 'Ask' y 'Agent'). La modalidad 'Plan' nos permite generar un plan de trabajo a partir de un prompt, la modalidad 'Ask' nos permite hacer preguntas sobre el código, y la modalidad 'Agent' propone modificaciones de código, ejecuta scritps (incluso scripts de prueba) y realiza actividades más allá del lenguaje R (como ejecutar comandos de cmd, powershell, bash, buscar información relevante en la web, etc.).
