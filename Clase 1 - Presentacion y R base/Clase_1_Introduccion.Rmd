...
Este curso busca dar las herramientas básicas para el procesamietno de estadísticas socio-económicas y laborales con R, y la visualización de resultados con PowerBI y Looker. Como verán en el programa, el curso es *muy ambicioso* y cubre una amplia variedad de herramientas. El obetivo del curso es conocer las potencialidades de todas estas herramientas para que ustedes luego profundicen según su interés o necesidades. Empecemos primero con el lenguage de programación R!
...
La actividad de programar cambió drásticamente en los últimos años debido al desarrollo de modelos de lenguaje basados en IA. Actualmente, el foco del aprendizaje ya no debe estar puesto en conocer en profundidad los comandos, funciones y operadores de R sino más bien en saber qué es posible hacer con R y como aplicar las soluciones a los problemas que se nos presentan. Las herramientas más utilizadas actualmente son: 

- *ChatGPT*: conocidísimo modelo desarrollado por OpenAI del cual no hace falta hacer ninguna introducción. Sólo tener en cuenta que, cuando traabajemos con R, debemos darle contexto al chat para que pueda darnos mejores respuestas. El contexto puede ser nuestro script entero o los mensajes que nos salen por consola (como por ejemplo cuando tenemos un error o warning que queremos evitar). También es una buena práctica dar una introducción en forma de texto sobre la actividad que estamos haciendo al menos una vez en un chat.

- *Github copilot*: herramienta desarrollada por Github que, si bien no da respuestas tan buenas como ChatGPT, tiene integraciones muy cómodas con IDEs. La principal ventaja de esta herramienta es que 'autocompleta' código con sugerencias que muchas veces parecen 'adivinar' lo que queremos hacer! En Visual Studio Code, se abre un 'in-line chat' que nos permite dar prompts en el mismo espacio donde estamos escribiendo código. En RStudio también hay disponible una integración que autocompleta código y toma los cometnarios que hacemos con "#" como si fueran prompts. La principal desventaje de GitHub copilot es que cuesta U$S 10 por mes, auqnue hay una prueba gratuita de un mes. 
