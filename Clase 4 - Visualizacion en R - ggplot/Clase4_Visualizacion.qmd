...
Para cerrar, veamos rápidamente la Muestra Longitudinal de Empleo Registrado (MLER) para hacer un gráfico de evolución del salario promedio del sector privado registrado. Dado el gran tamaño de la base de datos, para este ejercicio dejamos una sub-base del período 2016:2021 (la base original contiene información desde el 1996)

A diferencia de la EPH, esta infomación está construída como una base de datos de panel. Observen acá el formato de la base:

-   Cuenta con una variable **id_trabajador** que permite identificar a un mismo trabajador (anonimizadamente) a lo largo de la base de datos

-   Otra variable **id_relacion** que permite distinguir distintas relaciones laborales de un mismo trabajador

-   Una columna **tiempo** que identifica el período al que corresponde cada registro

-   **rem_tot**: La remuneración total percibida por dicha relación laboral

-   Un set acotado de variables suplementarias (identificadores que permiten tamaño de empresa, provincia, rama de actividad)

-   **pondera:** cantidad de acsos que representa cada fila (pese a que la información proviene de registros administrativos y no de una encuesta, la base de datos es una muestra del 3,33% de las relaciones totales registradas)

```{r}
mler <- read_rds("Fuentes/mler.rds")

head(mler)
```

Calculemos el salario promedio por mes en términos nominales.

```{r}
salario_promedio  <- mler %>%
  group_by(tiempo) %>%
  summarise(sal_prom = weighted.mean(rem_tot,pondera))
```

Utilicemos ahora una serie mensual del índice de precios (con base de 100 en Enero de 2006) al consumidor, para transformar el salario a valores reales. Para ello: - Creamos una variable *tiempo* con el mismo formato en el que viene en la base de MLER. - Aplicamos un left join entre nuestro objeto *salario_promedio* y nuestro objeto *ipc_formateado*. - Dividimos el salario nominal por el valor del IPC y multiplicamos por 100. Así nos queda una serie de salario reales (expresada a \$ de Enero de 2006) - Creamos una variable *periodo* que transforme la variable tiempo a formato fecha. Usamos la función *ym* del paquete lubridate

```{r}
ipc <- openxlsx::read.xlsx("Fuentes/ipc_mensual.xlsx")

ipc_formateado <- ipc %>% 
  mutate(tiempo = as.numeric(paste0(ANO4,sub)))

salario_ipc <- left_join(salario_promedio,ipc_formateado) %>% 
  mutate(salario_real = sal_prom/valor_ipc*100,
         periodo = lubridate::ym(tiempo)) 

```

-   Para gráficos de línea podemos usar la función *geom_line()*

Importante: La base de MLER incluye la remuneración por todo concepto, por tanto, tenemos el aguinaldo incluído en el salario total calculado.
...
ggplot(.,
  ...
  geom_point()+
  geom_line()+
  labs(title = "Salario promedio del sector privado no registrado",
       subtitle = "MLER",
       y = "Salario promedio")+
  theme_minimal()
```
