...
library(plotly)
library(tidyverse)
base <- get_microdata(year = 2023,period = 3:4)
...
precariedad_rama <- base %>%
  ...
  mutate(tasa_no_reg = round(tasa_no_reg,digits = 1),
         ...
         periodo = paste(ANO4,TRIMESTRE,sep = "_")) %>% 
  ungroup()
...
```{r}
 selectInput(
   inputId = "trimestre", label = "Elegir Trimestre",
   choices = unique(precariedad_rama$periodo),
   selected = unique(precariedad_rama$periodo)[1])
```
...
grafico_ramas <- precariedad_rama %>% 
  filter(periodo == input$trimestre) %>% 
    ggplot(mapping = aes(x = tasa_no_reg,
