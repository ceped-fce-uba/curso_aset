...
Trabajaremos con múltiples bases de la EPH - Argentina. 

```{r message=FALSE, warning=FALSE}
library(tidyverse)
library(openxlsx)
library(eph)
bases_2019_2023 <- eph::get_microdata(year = 2019:2023,period =1:4)
ipc_trimestra <- read.xlsx(xlsxFile = "bases/ipc_trimestral.xlsx")
```
...
```{r}
ultima.actualiz.base<- unique(bases_2019_2023 %>% select(ANO4,TRIMESTRE))
...
if (ultima.actualiz.base == Sys.Date()) {
  ...
  paste("Datos actualizados al", ultima.actualiz.base) 
}
```
