...
```{r}
vector_calificaciones <- c("Profesionales","Técnicos","Operativos","No calificados")
```
...
Supongamos que queremos un cuadro que nos muestre la cantidad de casos, según `calificacion` y `Sexo` (CH04).
...
openxlsx::write.xlsx(x = cuadro_por_calificaciones,
...
addWorksheet(wb,sheetName =  "Sexo-Calif") 

writeData(wb,  
          sheet="Sexo-Calif", 
          x = "Cuadro 1. Cantidad de Casos Sexo y Calificacion", #Lo uso para crear un titulo
          startRow = 1)


writeData(wb, 
          sheet="Sexo-Calif",
          x = cuadro_por_calificaciones,   #Acá va la base de datos
          borders = "rows" ,#Parametro de Estilo
          borderStyle = "dashed", #Parametro de Estilo
          startRow = 3) #Dejo un espacio entre titulo y datos

saveWorkbook(wb, "Resultados/miexportacion_formateada.xlsx")
...
calificaciones <- unique(cuadro_por_calificaciones$CALIFICACION)
...
for (valor in calificaciones) {
  ...
  #Este cuadro ira cambiando de calificación a medida que avanza el loop
cuadro.calif <- cuadro_por_calificaciones %>%  
    filter(CALIFICACION == valor)
  
addWorksheet(wb, sheetName =  valor) #Al poner "valor" estoy agregando una pestaña con cada nombre de cada calificación 
