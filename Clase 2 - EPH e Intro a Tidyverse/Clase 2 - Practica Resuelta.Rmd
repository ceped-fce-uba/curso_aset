...
- Ejercicio 1: Calcular para el 1er trimestre de 2025 el conjunto de tasas básicas del mercado de trabajo que figuran en el Cuadro 1.1 del [Informe técnico elaborado por EPH-INDEC](https://www.indec.gob.ar/uploads/informesdeprensa/mercado_trabajo_eph_1trim254850EDE6DF.pdf). 

```{r echo=TRUE}
# Paso 1 levantar las bases de EPH y seleccionar las variables de interés
library(tidyverse)
indiv_t1_25 <-
  read.table("Fuentes/usu_individual_T125.txt",
  sep = ";",
  dec = ",",
  header = TRUE,
  fill = TRUE )
...
regiones <- readxl::read_excel("Fuentes/Aglomerados y Regiones EPH.xlsx",sheet = 3)
```
...
```{r}
tasas_regiones_sexo <- indiv_t1_25 %>% 
...
- Ejercicio 3: Calcular, al mismo tiempo para  2 trimestres (1T de 2025 y 1T de 2017), las tasas de actividad, empleo y desempleo según sexo, para la población de edad entre 18 y 35 años.    
...
base_t1_25_select<- indiv_t1_25[,variables]
base_t1_17_select <- individual_t1_17[,variables]

base_unida <- bind_rows(base_t1_17_select,base_t1_25_select)
```
...
resolucion <- base_unida %>% 
  ...
  ungroup()
```
- Ejercicio 4: Elegir un trimestre y calcular el salario promedio por sexo, para dos grupos de edad: 18 a 35 años y 36 a 70 años. (Recordatorio: La base debe filtrarse para contener únicamente OCUPADOS ASALARIADOS)
```{r}
ultimo.ejercicio.resol <- indiv_t1_25 %>%   
  filter(ESTADO==1) %>% 
  filter(CAT_OCUP==3) %>% 
...
openxlsx::write.xlsx(x = ultimo.ejercicio.resol,"Resultados/exportacion.xlsx")
```
